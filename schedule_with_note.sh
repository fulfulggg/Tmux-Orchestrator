...
nohup bash -c "sleep $SECONDS && tmux send-keys -t $TARGET 'Time for orchestrator check! cat /Users/jasonedward/Coding/Tmux\ orchestrator/next_check_note.txt' && sleep 1 && tmux send-keys -t $TARGET Enter" > /dev/null 2>&1 &
